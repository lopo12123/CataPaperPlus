...
```bash
git clone https://github.com/lopo12123/CataPaperPlus.git
cd CataPaperPlus

conda create -n CataPaperPlus python=3.10
conda activate CataPaperPlus
...
`CataPaperPlus` provides a `RAG` integration interface, allowing users to specify a single input `pdf` file or a directory. `CataPaperPlus` will automatically parse the input files and return an iterable interface for retrieving the data.
