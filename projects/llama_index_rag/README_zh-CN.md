...
`CataPaperPlus` 提供数据 `API接口` 以支持用户导入数据到 `RAG` 系统。本项目将基于`通义千问`展示如何构建一个轻量级的 `RAG` 系统。
...
请参考[文档](../../README_zh-CN.md) 安装 CataPaperPlus
...
`CataPaperPlus` 提供了 `RAG` 集成接口，用户可以通过指定输入单个 `pdf` 文件或者某个目录。`CataPaperPlus` 会自动解析输入文件并返回可以迭代的接口用于获取数据
